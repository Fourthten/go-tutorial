...
type AuthRepository interface {
	...
	RefreshTokenExist(refreshToken string) *errs.AppError
}
...
func (d AuthRepositoryDb) RefreshTokenExist(refreshToken string) *errs.AppError {
...
func (d AuthRepositoryDb) FindBy(username, password string) (*Login, *errs.AppError) {
	...
	sqlVerify := `SELECT username, u.customer_id, role, group_concat(a.account_id) as account_numbers FROM users u 
		LEFT JOIN accounts a ON a.customer_id = u.customer_id WHERE username = ? and password = ? GROUP BY a.customer_id`
