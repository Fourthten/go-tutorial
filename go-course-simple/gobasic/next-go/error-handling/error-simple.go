	...
	e.HTTPErrorHandler = func(err error, c echo.Context) {
		...
		if castedObject, ok := err.(validator.ValidationErrors); ok {
			for _, err := range castedObject {
				...
				case "gte":
					report.Message = fmt.Sprintf("%s value must be grater than %s", err.Field(), err.Param())
				case "lte":
...
// curl -X POST http://localhost:9000/users -d 'name=Setia' -d 'email=test@mail.com' -d 'age=11'
